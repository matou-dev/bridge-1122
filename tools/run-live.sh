...
#    The narrow map grows 54 -> 56 lines (48 server rows plus the 8
#    renderer rows below).
...
# tools/live/want.tsv — same 56 lines, byte-identical output.
...
[ "$(grep -c . "$SRG_NARROW")" = "56" ] \
  || { echo "FAIL c3-live : narrow map drift (want 56 lines)"; exit 1; }
...
# tranche — proof-only, never shipped in dist/). TRIPLE_GEO overlays the
# triple-bone index-2 proof asset the same way (palette index-2 tranche —
# proof-only, never shipped in dist/).
...
# the geometry, operator-replaceable like it. TRIPLE_ANIM overlays the
# triple-bone proof clip the same way (palette index-2 tranche —
# proof-only, never shipped in dist/).
...
live_verdict "NoSuchMethodError\|NoSuchFieldError\|E_FORGE\|E_BRIDGE\|E_EXAMPLE\|E_REG\|E_LOOT\|E_SPAWN\|E_MODEL\|E_HIT\|E_ANIM\|Encountered an unexpected exception" "NoSuchMethodError\|NoSuchFieldError\|E_FORGE\|E_BRIDGE\|E_EXAMPLE\|E_REG\|E_LOOT\|E_SPAWN\|E_MODEL\|E_HIT\|E_ANIM\|Caused by" $LOGS
