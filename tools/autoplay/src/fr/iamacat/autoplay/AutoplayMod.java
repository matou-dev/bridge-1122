 ...
 * honesty standard), five ticks later kills the registered
 * {@code my_beast} at (12,10,8) and five ticks after that the
 * registered {@code my_brute} at (14,10,8), each with a simulated 5-arg
 * {@code LivingDropsEvent} post carrying that mob's explicit identity
 * (distinct-drops tranche, hub decisions/LOOT.md — same triple as the
 * etage-1 gate) — then polls the three spots for the per-mob carriers
 * the bridge loot sink spawns per due drop (ore 1x {@code my_gem},
 * beast 1x {@code my_gem}, brute 2x {@code my_brute_gem}). The posts
...
public class AutoplayMod {
     ...
     * teleported player stands, motionY 0 — a crit would fail the exact
     * assert loudly, never pass as a weakspot). Without SPAWN=1 nothing
     * runs (COMBAT=1 alone fails loudly — no beasts to strike); without
    ...
    volatile int killTick = -1;    volatile int killX = 0;
    ...
    private void lootPoll() {
        ...
        Item bruteGem = Item.getByNameOrId("example1:my_brute_gem");
        if (bruteGem == null) {
    ...
    private void spawnTick() {
        ...
        for (Entity e : world.loadedEntityList) {
            ...
            pigs++;
            String mob = ((MatouEntity) e).mobOrFirst();
            if ("my_brute".equals(mob)) {
                brutes++;
                if (firstBrute == null) {
                    firstBrute = (MatouEntity) e;
                }
            } else {
                beasts++;
                if (firstBeast == null) {
                    firstBeast = (MatouEntity) e;
                }
            }
        }
        if (pigs > maxPigs) {
     ...
     * members go through the declaring stub type, never the beast —
     * {@code Entity} for positions/eye/teleport, {@code EntityPlayer}
     * for the strike itself (declared there), {@code EntityLivingBase}
     * for the health read. Per-mob: {@code mob} names the victim
     * (my_beast head x2, my_brute head x3 — the exact poll owns the
     * multiplier, never a quiet pick here).
     */
    private void combatAttack(MatouEntity beast, String mob) {
        int strikeTick = "my_brute".equals(mob) ? COMBAT_TICK_BRUTE : COMBAT_TICK;
        if (world.playerEntities == null
                || world.playerEntities.isEmpty()) {
            if (worldTicks > strikeTick + COMBAT_TIMEOUT) {
                combatFail("player never joined (no strike author <" + mob + ">)");
            }
            return;
        }
        ...
        // MC yaw faces +Z at 0 (-90 at +X, measured against getLookVec):
        ...
        // getLookVec — the bridge hook reads the real one; this only
