...
public class AutoplayMod {
     ...
     * 2.0 (crit excluded: the teleported player stands, motionY 0 —
     * a crit would fail the exact assert loudly, never pass as a
     * weakspot). Without SPAWN=1 nothing runs (COMBAT=1 alone fails
     * loudly — no beasts to strike); without COMBAT=1 the run is
     * byte-for-byte the proven spawn run. */
